...
function loadVaccinations() {
    onValue(ref(db, 'vaccinations'), (snapshot) => {
        const data = snapshot.val();
...
milestoneForm.onsubmit = (e) => {
    ...
    id ? update(ref(db, `milestones/${id}`), data) : push(ref(db, 'milestones'), data);
...
vaccinationForm.onsubmit = (e) => {
    ...
    id ? update(ref(db, `vaccinations/${id}`), data) : push(ref(db, 'vaccinations'), data);
